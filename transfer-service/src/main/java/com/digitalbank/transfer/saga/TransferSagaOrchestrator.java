...
public class TransferSagaOrchestrator {
    ...
    public Transfer initiateTransfer(Long fromAccountId, Long toAccountId, BigDecimal amount) {
        ...
        log.info(
                "Transfer initiated id={} fromAccountId={} toAccountId={} amount={}",
                transfer.getId(),
                fromAccountId,
                toAccountId,
                amount
    ...
    public void onStepResult(TransferStepResult result) {
        ...
        log.info(
                "Saga received result transferId={} commandId={} type={} status={} currentState={}",
                result.transferId(),
                result.commandId(),
                result.type(),
                result.status(),
    ...
    private void publish(String topic, Long transferId, Object payload) {
        try {
            ...
            throw new IllegalStateException("Could not publish transfer message to " + topic, ex);
        }
    }

    private void logDuplicateOrStaleResult(Transfer transfer, TransferStepResult result) {
        log.info(
                "Ignoring duplicate or stale result commandId={} type={} for transferId={} in state={}",
                result.commandId(),
                result.type(),
                transfer.getId(),
                transfer.getStatus()
